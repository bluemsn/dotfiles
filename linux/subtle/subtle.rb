# # author::  spo11, modified by Eduan
# Version:: 1.1g
# License:: GNU GPL
#
# = custom subtle
...
#set :font, "-*-*-medium-*-*-*-12-*-*-*-*-*-*-*"
set :font, "xft:DejaVu Sans Mono:pixelsize=9"
...
screen 1 do
  ...
  top  [ :views, :spacer, :title, :spacer, :tray, :separator, :wifi, :battery, :volume, :separator, :clock ]

  # Content of the bottom panel
  #bottom  [ :views, :spacer, :title, :spacer, :tray, :separator, :wifi, :battery, :volume, :separator, :clock ]
end
...
tag "browser", "luakit|jumanji|uzbl|firefox|chromium|opera|navigator"
tag "dev", "emacs|gvim|lyx|subl|subl3"
...
# Placement
tag "editor" do
  match  "medit|geany|gedit|pyroom|gvim|cvim|emacs|subl|subl3"
  resize false
end
...
# hooks
on :start do
   Subtlext::Subtle.spawn "setxkbmap -layout 'latam' -variant 'deadtilde'"
end
on :start do
   Subtlext::Subtle.spawn "dropboxd &"
end
on :start do
   Subtlext::Subtle.spawn "xbindkeys"
end
on :start do
   Subtlext::Subtle.spawn "touchegg &"
end
